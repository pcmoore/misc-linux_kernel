				       ...
				       bool may_sleep)
{
	...
	if (selinux_initialized(current_selinux_ns) &&
...
#define INITCONTEXTLEN 255
	...
	rc = security_context_to_sid_default(current_selinux_ns, context, rc,
					...
					struct kernfs_node *kn)
{
	...
	rc = security_context_to_sid(current_selinux_ns, context, clen,
...
static __init int selinux_init(void)
{
	...
	memset(&init_selinux_ns, 0, sizeof(init_selinux_ns));
	...
	mutex_init(&init_selinux_ns.status_lock);
...
{
	if (selinux_initialized(ns)) {
		/* Not permitted after initial policy load. */
		return -EINVAL;
	}

	if (selinux_disabled(ns)) {
		/* Only do this once. */
		return -EINVAL;
	}

	selinux_mark_disabled(ns);
