...
static inline bool selinux_initialized(const struct selinux_ns *ns)
{
	...
	return smp_load_acquire(&ns->initialized);
}

static inline void selinux_mark_initialized(struct selinux_ns *ns)
{
	/* do a synchronized write to avoid race conditions */
	smp_store_release(&ns->initialized, true);
}

#ifdef CONFIG_SECURITY_SELINUX_DEVELOP
static inline bool enforcing_enabled(struct selinux_ns *ns)
{
	return READ_ONCE(ns->enforcing);
}

static inline void enforcing_set(struct selinux_ns *ns, bool value)
{
	WRITE_ONCE(ns->enforcing, value);
}
#else
static inline bool enforcing_enabled(struct selinux_ns *ns)
{
...
#ifdef CONFIG_SECURITY_SELINUX_DISABLE
static inline bool selinux_disabled(struct selinux_ns *ns)
{
	return READ_ONCE(ns->disabled);
}

static inline void selinux_mark_disabled(struct selinux_ns *ns)
{
	WRITE_ONCE(ns->disabled, true);
}
#else
static inline bool selinux_disabled(struct selinux_ns *ns)
{
...
static inline bool selinux_policycap_genfs_seclabel_symlinks(void)
{
	...
	return ns->policycap[POLICYDB_CAPABILITY_GENFS_SECLABEL_SYMLINKS];
}
